...
function LoadingFallback({ label }: { label: string }) {
  return (
    <div className="mfe-loading" role="status" aria-label={`Loading ${label}`}>
      <span className="mfe-loading__dot" />
      <span className="mfe-loading__dot" />
      <span className="mfe-loading__dot" />
    </div>
  )
...
function ErrorFallback({ name }: { name: string }) {
  return (
    <div className="mfe-error" role="alert">
      <p>Could not load <strong>{name}</strong>.</p>
      <p className="mfe-error__hint">Make sure the remote is running on the correct port.</p>
    </div>
  )
}
  ...
  return (
    <section className="home">
      ...
      <p className="home__sub">
        This portfolio is a micro-frontend shell — the Projects section runs
        in <strong>React</strong>, the About section in <strong>Vue</strong>,
